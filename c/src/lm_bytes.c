...
// [-1, +1, e]
int lcf_bytes_read_from_fd(lua_State *L) {
...
// [-1, +1, e]
static int lcf_bytes_ref_get_length(lua_State *L) {
  as_lm_bytes_ref_t *br = (as_lm_bytes_ref_t *)luaL_checkudata(L, 1, LM_BYTES);
  lua_pushinteger(L, br->len);
  return 1;
}

// [-1, +1, e]
static int lcf_bytes_get_length(lua_State *L) {
...
// [-1, +0, e]
static int lcf_bytes_destroy(lua_State *L) {
  as_bytes_t *bs = (as_bytes_t *)luaL_checkudata(L, 1, LM_BYTES);
  bytes_destroy(bs);
  return 0;
}

static const struct luaL_Reg as_lm_bytes_methods[] = {
...
static const struct luaL_Reg as_lm_bytes_ref_methods[] = {
    {"get_length", lcf_bytes_ref_get_length},

    {NULL, NULL}};

void inc_lm_bytes_metatable(lua_State *L) {
