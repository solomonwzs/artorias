...
#define th_remove_res_from_epfd(_th_, _epfd_)      \
  do {                                             \
...
static inline void th_yield_for_io(as_thread_t *th, as_mw_worker_ctx_t *ctx) {
  lua_State *T = th->T;
  ...
  uint32_t events =
      io_type == LAS_S_WAIT_FOR_INPUT ? EPOLLIN | EPOLLET : EPOLLOUT | EPOLLET;
  asthread_res_ev_add(res, ctx->epfd, events);
}

static inline void th_yield_for_sleep(as_thread_t *th,
...
static inline void th_yield_for_ev(as_thread_t *th, as_mw_worker_ctx_t *ctx) {
  lua_State *T = th->T;
...
static void thread_resume(as_thread_t *th, as_mw_worker_ctx_t *ctx, int nargs) {
...
static void handle_accept(as_mw_worker_ctx_t *ctx, int single_mode) {
  int (*new_fd_func)(int);
  if (single_mode) {
...
static void handle_fd_read(as_mw_worker_ctx_t *ctx, as_thread_res_t *res) {
...
static void handle_fd_write(as_mw_worker_ctx_t *ctx, as_thread_res_t *res) {
  if (asthread_th_is_stop(res->th)) {
    return;
  }
  asthread_th_del_from_pool(res->th);
...
static void handle_fd_error(as_mw_worker_ctx_t *ctx, as_thread_res_t *res) {
  if (res == ctx->cfd_res) {
...
static void process_stop_threads(as_mw_worker_ctx_t *ctx) {
...
static inline void p_io_timeout_thread(as_rb_node_t *n,
                                       as_mw_worker_ctx_t *ctx) {
...
static void process_io_timeout(as_mw_worker_ctx_t *ctx) {
...
static void process_io(as_mw_worker_ctx_t *ctx, int single_mode) {
...
static inline void p_sleep_thread(as_rb_node_t *n, as_mw_worker_ctx_t *ctx) {
  as_thread_t *th = rb_node_to_thread(n);
  if (asthread_th_is_stop(th)) {
    return;
  }
...
static void process_sleep(as_mw_worker_ctx_t *ctx) {
  as_rb_node_t *timeout_tr = asthread_remove_timeout_threads(&ctx->sleep_pool);
  rb_tree_postorder_travel(timeout_tr, p_sleep_thread, ctx);
}

static inline void r_thread_from_pool(as_rb_node_t *n) {
  as_thread_t *th = rb_node_to_thread(n);
...
static void recycle_threads_from_pool(as_mw_worker_ctx_t *ctx) {
...
static void process(int cfd, as_lua_pconf_t *cnf, int single_mode) {
  set_non_block(cfd);
  if (single_mode) {
  ...
  size_t fs[] = {12,  16,  24,  32,  48,   64,   128,
                 256, 384, 512, 768, 1064, 2048, 4096};
...
void worker_process(int channel_fd, as_lua_pconf_t *cnf) {
  process(channel_fd, cnf, 0);
}

void test_worker_process(int fd, as_lua_pconf_t *cnf) {
  process(fd, cnf, 1);
}
